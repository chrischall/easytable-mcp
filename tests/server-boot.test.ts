...
// eager-import crash in the bundle (run with NO node_modules, the .mcpb runtime)
// and a wrong `bin` path — neither of which the mocked unit tests see.
...
function listToolsViaStdio(entry: string, cwd: string): Promise<string[]> {
  return new Promise((resolve, reject) => {
    const child = spawn('node', [entry], {
      cwd,
      // Port 0 so the boot test never fights the real 37149 bridge; the server
      // binds lazily on first request, not at boot, so this is belt-and-braces.
      env: { ...process.env, EASYTABLE_WS_PORT: '0' },
...
// added on other branches (a PR is CI-tested merged with main). Pinned to the
// current roster size (8) so a tool silently dropping still trips this — index.test.ts
// owns the exact roster; additions only ever push the real count above the floor.
const MIN_TOOLS = 8;

describe('server boot (built artifacts)', () => {
  it('bundled .mcpb (dist/bundle.js) boots WITHOUT node_modules and lists tools', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'easytable-mcpb-'));
    try {
      ...
      expect(tools).toContain('easytable_healthcheck');
      expect(tools).toContain('easytable_list_times');
    } finally {
